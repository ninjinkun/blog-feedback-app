      ...
      <ContentWrapper>
        ...
        <Paragraph>
          ...
          　<DecimalList>
            ...
            <DecimalList>ユーザーが本サービスを利用するにあたって、サービス提供者が収集する情報
              <Paragraph>サービス提供者は、本サービスへのアクセス状況やそのご利用方法に関する情報を収集することがあります。これには以下の情報が含まれます。
                <UnorderedList>
                  <List>ログ情報</List>
        ...
        <OrderedList>
          ...
          <DecimalList>
            ...
            <OrderedList>
            　<RomanList>本サービスに関する登録の受付、本人確認、利用料金の計算等本サービスの提供、維持、保護及び改善のため</RomanList>
            　<RomanList>本サービスに関するご案内、お問い合せ等への対応のため</RomanList>
            　<RomanList>本サービスに関するサービス提供者の規約、ポリシー等（以下「規約等」といいます。）に違反する行為に対する対応のため</RomanList>
          ...
          <DecimalList>
            ...
            <Paragraph>
              <Table>
                <thead>
                  <tr><TableHeader>利用目的</TableHeader><TableHeader>対応する利用者情報の項目</TableHeader></tr>
                </thead>
                <tr>
                  <TableData>サービス提供者のサービスに関連して、個人を識別できない形式に加工した統計データを作成するため</TableData>
                  <TableData>
                    <UnorderedList>
                      <List>ログ情報</List>
                      <List>Cookie及び匿名ID</List>
        ...
        <Paragraph>サービス品質向上の目的で利用者情報を分析するため、本サービスには以下の情報収集モジュールが組み込まれています。これに伴い、以下のとおり情報収集モジュール提供者への利用者情報の提供を行います。</Paragraph>
        <Paragraph>
          <Table>
            <tr>
            ...
            <tr>
              ...
              <TableData>以下の情報がユーザー個人が特定されない統計データとして送信されます。 
                <UnorderedList>
                  <List>本サービスのご利用状況（本サービス訪問回数、滞在時間、閲覧ページ数、画面遷移等）</List>
                  <List>ご利用端末・ブラウザの情報（端末の設定言語、地域、機種名、OS、ブラウザバージョン、プロバイダ等</List>
                </UnorderedList>
              </TableData>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <TableData>サービス品質向上のために役立つ情報を記憶し、本サービスを最適な状態で利用していただく目的のみで使用します。</TableData>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <TableData>デスクトップ向けブラウザをご利用の場合はGoogleが提供するオプトアプトアウドオン (<Anker href="https://tools.google.com/dlpage/gaoptout">https://tools.google.com/dlpage/gaoptout</Anker>) を導入する事で無効化が可能です。</TableData>
            </tr>
            <tr>
              <TableData>上記提供者のプライバシーポリシーのURL</TableData>
              <TableData><Anker href="https://policies.google.com/privacy?hl=ja">https://policies.google.com/privacy?hl=ja</Anker></TableData>
            </tr>
          </Table>
        </Paragraph>

        <Title>5.第三者提供</Title>
        <Paragraph>サービス提供者は、利用者情報のうち、個人情報については、個人情報保護法その他の法令に基づき開示が認められる場合を除くほか、あらかじめユーザーの同意を得ないで、第三者に提供しません。但し、次に掲げる場合はこの限りではありません。</Paragraph>
        <OrderedList>
          <DecimalList>サービス提供者が利用目的の達成に必要な範囲内において個人情報の取扱いの全部または一部を委託する場合</DecimalList>
          <DecimalList>合併その他の事由による事業の承継に伴って個人情報が提供される場合</DecimalList>
          <DecimalList>第4項の定めに従って、情報収集モジュール提供者へ個人情報が提供される場合</DecimalList>
          <DecimalList>国の機関もしくは地方公共団体またはその委託を受けた者が法令の定める事務を遂行することに対して協力する必要がある場合であって、ユーザーの同意を得ることによって当該事務の遂行に支障を及ぼすおそれがある場合</DecimalList>
        ...
        <Paragraph>サービス提供者は、ユーザーから、個人情報保護法の定めに基づき個人情報の開示を求められたときは、ユーザーご本人からのご請求であることを確認の上で、ユーザーに対し、遅滞なく開示を行います（当該個人情報が存在しないときにはその旨を通知いたします）。但し、個人情報保護法その他の法令により、サービス提供者が開示の義務を負わない場合は、この限りではありません。なお、個人情報の開示につきましては、手数料（1件あたり1,000円）を頂戴しておりますので、あらかじめ御了承ください。</Paragraph>

        <Title>8.個人情報の訂正及び利用停止等</Title>
        <OrderedList>
          <DecimalList>サービス提供者は、ユーザーから、⑴個人情報が真実でないという理由によって個人情報保護法の定めに基づきその内容の訂正を求められた場合、及び⑵あらかじめ公表された利用目的の範囲を超えて取り扱われているという理由または偽りその他不正の手段により収集されたものであるという理由により、個人情報保護法の定めに基づきその利用の停止を求められた場合には、ユーザーご本人からのご請求であることを確認の上で遅滞なく必要な調査を行い、その結果に基づき、個人情報の内容の訂正または利用停止を行い、その旨をユーザーに通知します。なお、合理的な理由に基づいて訂正または利用停止を行わない旨の決定をしたときは、ユーザーに対しその旨を通知いたします。</DecimalList>
          <DecimalList>サービス提供者は、ユーザーから、ユーザーの個人情報について消去を求められた場合、サービス提供者が当該請求に応じる必要があると判断した場合は、ユーザーご本人からのご請求であることを確認の上で、個人情報の消去を行い、その旨をユーザーに通知します。</DecimalList>
          <DecimalList>個人情報保護法その他の法令により、サービス提供者が訂正等または利用停止等の義務を負わない場合は、前2項の規定は適用されません。</DecimalList>
        </OrderedList>

        <Title>9.お問い合わせ窓口</Title>
        <Paragraph>ご意見、ご質問、苦情のお申出その他利用者情報の取扱いに関するお問い合わせは、下記の窓口までお願い致します。</Paragraph>
        <UnorderedList>
          <List>mail: <Anker href="mail:ninjinkun+feedback@gmail.com">ninjinkun+feedback@gmail.com</Anker></List>
        </UnorderedList>

        <Title>10.プライバシーポリシーの変更手続</Title>
        <Paragraph>サービス提供者は、利用者情報の取扱いに関する運用状況を適宜見直し、継続的な改善に努めるものとし、必要に応じて、本ポリシーを変更することがあります。変更した場合には、本サイトに掲示する方法でユーザーに通知いたします。但し、法令上ユーザーの同意が必要となるような内容の変更の場合は、サービス提供者所定の方法でユーザーの同意を得るものとします。</Paragraph>
      </ContentWrapper>
    </StyledScrollView>
  </PageLayout>
);
